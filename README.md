...
GitHub Pages hosts your blog for free at `https://YOUR_USERNAME.github.io`.
...
4. **Access your site:**
   - Your blog will be available at: `https://m42839360-cell.github.io`
   - URL shown in the Pages settings after successful deployment
...
- `url: "https://m42839360-cell.github.io"` - Base URL for your site
- `baseurl: ""` - No subpath (root deployment)
- `jekyll-feed` plugin enabled for RSS

✅ **Gemfile** uses `github-pages` gem:
- Ensures compatibility with GitHub Pages environment
- Includes all necessary Jekyll plugins
- Matches GitHub Pages gem versions

✅ **Excluded files** (`.github/`, `backlog/`, etc.):
- Automation files excluded from site build
- Only content and templates are published
...
**Note:** After running `bundle install` with the updated Gemfile, your Gemfile.lock will update to use GitHub Pages gem versions.

### How Publishing Works

**Manual posts** (if you create them):
...
**Automated posts** (via GitHub Actions):
1. Workflow runs on schedule or manual trigger
2. Generates post in `_posts/` directory
...
Your blog includes an RSS feed at:
- `https://m42839360-cell.github.io/feed.xml`
- Automatically generated by `jekyll-feed` plugin
- Updates with each new post

### Custom Domain (Optional)

To use a custom domain (e.g., `blog.yourdomain.com`):
...
2. **Configure DNS:**
   - Add CNAME record: `blog` → `m42839360-cell.github.io`
...
4. **Enable HTTPS** in repository settings (automatic after DNS propagation)
...
**404 errors:**
- Verify `url` in `_config.yml` matches your GitHub Pages URL
- Check that `baseurl` is empty (`""`) for root deployment
- Clear browser cache and try again

**Posts not showing:**
- Verify post filename format: `YYYY-MM-DD-title.md`
- Check frontmatter includes `layout: post`
- Ensure post date is not in the future

**Local build works but GitHub Pages fails:**
- Run `bundle update` to match GitHub Pages gem versions
- Check excluded files in `_config.yml`
